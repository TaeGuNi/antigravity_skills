...
- 🛡️ **보안(Security IAM) 인프라 특례**: `security_iam_protocol`에서 요구하는 "Refresh Token 무효화(Revoke) 블랙리스트", "IP Rate Limiting (Brute-Force 차단)" 기능을 구현하기 위해 Redis 등 외부 분산 저장소에 상태(State)를 쓰거나 읽는 행위는 **애플리케이션 비즈니스(App Tier) 계층의 오염된 튜링 상태(Turing State)가 아닌, 인프라 방어기제로 간주하여 특별 허용**합니다. (단, API 서버 메모리 자체를 쓴다는 뜻은 아닙니다)
